...
it('edits notes and estimates, starts with a snapshot, and persists manual priority', async () => {
  ...
  fireEvent.click(await screen.findByRole('button',{name:'Edit Task 1'}))
  ...
  fireEvent.click(screen.getByRole('button',{name:'Move Task 2 up'})); await waitFor(async () => expect((await db.tasks.get('2'))?.priorityOrder).toBe(0))
  fireEvent.click(screen.getByRole('button',{name:'Move Task 2 down'})); await waitFor(async () => expect((await db.tasks.get('2'))?.priorityOrder).toBe(1))
  view.unmount(); render(<TasksPage active={null} onStart={start}/>); await screen.findByRole('button',{name:'Edit Task 2'})
...
it('pages completed tasks and searches across statuses', async () => {
  ...
  render(<TasksPage active={null} onStart={vi.fn()}/>); await screen.findByRole('button',{name:'Edit Task 0'})
...
it('deletes with confirmation, retains historical sessions and supports Undo', async () => {
  ...
  render(<TasksPage active={null} onStart={vi.fn()}/>);fireEvent.click(await screen.findByRole('button',{name:'Edit Task 1'}));fireEvent.click(screen.getByRole('button',{name:'Delete task'}));expect(await db.tasks.get('1')).toBeDefined();fireEvent.click(screen.getByRole('button',{name:'Confirm delete'}))
  ...
  fireEvent.click(within(screen.getByRole('status')).getByRole('button',{name:'Undo'}));await screen.findByRole('button',{name:'Edit Task 1'});expect((await db.sessions.get('log'))?.taskId).toBe('1')
})
