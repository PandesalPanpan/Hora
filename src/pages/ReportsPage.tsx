...
export function ReportsPage({ completed, onOpenHistory }: { completed: CompletedSession[]; onOpenHistory: (date: string) => void }) {
  ...
  return <section className="hora-page insights-page">
    ...
    <section className="weekly-chart"><header><h2>Select a day</h2><strong>{formatCompactDuration(summary.actualTotal)}</strong></header><div>{summary.days.map((day, index) => <button type="button" key={localDateKey(day)} className={localDateKey(day) === selectedKey ? 'selected' : ''} onClick={() => setSelectedKey(localDateKey(day))} aria-label={`Show ${day.toLocaleDateString(undefined, { dateStyle: 'full' })}`}><i style={{ height: summary.actualByDay[index] ? `${summary.actualByDay[index] / max * 88}px` : '0' }} /><small>{day.toLocaleDateString(undefined, { weekday: 'narrow' })}</small></button>)}</div></section>
    <section className="time-breakdown"><h2>{summary.days[selectedIndex].toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</h2><div className="report-comparison"><span><small>Planned</small><strong>{formatCompactDuration(summary.plannedByDay[selectedIndex])}</strong></span><span><small>Actually tracked</small><strong>{formatCompactDuration(summary.actualByDay[selectedIndex])}</strong></span></div><p>{selectedSessions.length} completed {selectedSessions.length === 1 ? 'session' : 'sessions'}</p>{breakdown.length ? <div className="breakdown-list">{breakdown.map(([name, seconds], index) => <p key={name}><i style={{ background: colors[index % colors.length] }} /><span>{name}</span><b>{formatCompactDuration(seconds)}</b></p>)}</div> : <div className="soft-empty"><strong>No tracked time</strong><span>Finished Timeflow sessions for this day will appear here.</span></div>}<button className="history-day-link" type="button" onClick={() => onOpenHistory(selectedKey)}>Open this day in History</button></section>
  </section>
}
