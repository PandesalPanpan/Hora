...
const moodOrder = ['focused', 'calm', 'tired'] as const
const moodLabels = { calm: 'Calm', focused: 'Focused', tired: 'Tired' } as const
export function ReportsPage({ completed, onOpenHistory }: { completed: CompletedSession[]; onOpenHistory: (date: string) => void }) {
  ...
  const recordedMoods = moodOrder.filter((mood) => summary.moodCounts[mood] > 0)
  ...
  return <section className="hora-page insights-page">
    ...
    {recordedMoods.length > 0 && <section className="report-mood-summary" aria-label="Feelings noted this week"><h2>Feelings noted this week</h2><div>{recordedMoods.map((mood) => <span key={mood}>{moodLabels[mood]} <b>{summary.moodCounts[mood]}</b></span>)}</div></section>}
