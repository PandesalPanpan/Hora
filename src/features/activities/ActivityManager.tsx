...
export function ActivityManager({ onClose }: { onClose: () => void }) {
  ...
  const [colorSaveAttempted, setColorSaveAttempted] = useState(false)
  ...
  const edit = async (preset: ActivityPreset) => { setCounts(await activityCounts(preset.id)); setEverywhere(false); setHexInput(preset.color); setColorSaveAttempted(false); setEditing(preset) }
  const changeHexInput = (value: string) => {
    setHexInput(value)
    const color = normalizeHexColor(value)
    if (color) setEditing(current => current ? { ...current, color } : current)
  }
  const selectColor = (value: string) => {
    const color = normalizeHexColor(value)
    if (!color) return
    setHexInput(color)
    setEditing(current => current ? { ...current, color } : current)
  }
  ...
  return <div className="completion-backdrop"><section className="completion-sheet activity-manager" role="dialog" aria-modal="true" aria-label="Manage activities"><header><div><h1>Activities</h1><p>Drag activities to reorder them or move them into another category.</p></div><button className="sheet-close" type="button" onClick={onClose} aria-label="Close activities">×</button></header>{error && <p role="alert">{error}</p>}
    {editing ? <form onSubmit={event => { event.preventDefault(); const color = normalizeHexColor(hexInput); setColorSaveAttempted(true); if (!color) return; void run(async () => { await saveActivity({ ...editing, color }, everywhere); await reconcilePlannedBlockReminders().catch(() => undefined); setEditing(null) }) }}>
      ...
      <ActivityColorPicker activityName={editing.name} value={editing.color} inputValue={hexInput} showError={colorSaveAttempted} onInputValueChange={changeHexInput} onSelect={selectColor} />
      <label className="note-field">Apply changes to<select value={everywhere ? 'all' : 'activity'} onChange={event => setEverywhere(event.target.value === 'all')}><option value="activity">Activity only</option><option value="all">Activity and existing records</option></select></label>
      <p>Linked planned blocks always reflect this color.</p>
      {everywhere && <p>Also updates {counts.sessions} completed session{counts.sessions === 1 ? '' : 's'} and {counts.plans} linked planned-block categor{counts.plans === 1 ? 'y' : 'ies'}. A running timer keeps its Activity details; custom planned titles stay the same.</p>}
    ...
    </form> : <div className="activity-manager-body"><button className="save-log create-activity" type="button" onClick={() => { const color = '#D92F6F'; setEverywhere(false); setCounts({ sessions: 0, plans: 0 }); setHexInput(color); setColorSaveAttempted(false); setEditing({ id: crypto.randomUUID(), name: '', normalizedName: '', category: 'Focus', color, archived: false, order: presets.length, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }) }}>Create activity</button>
