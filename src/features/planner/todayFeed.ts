...
export function todayFeed(active: ActiveSession | null, completed: CompletedSession[], planned: PlannedBlock[], now = new Date(), limit = 3): TodayRow[] {
  ...
  const upcoming: TodayRow[] = planned.flatMap(expandRecurringBlock).filter(item => localDateKey(new Date(item.startedAt)) === key && +new Date(item.startedAt) >= +now).sort((a,b) => a.startedAt.localeCompare(b.startedAt)).map(item => ({ id: item.id, kind: 'Planned', title: item.title, note: item.note, color: item.color, startedAt: item.startedAt, durationSeconds: Math.max(0, Math.round((+new Date(item.finishedAt) - +new Date(item.startedAt)) / 1000)), destination: `${dayRoute}&block=${encodeURIComponent(item.id)}` }))
